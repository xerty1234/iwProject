...
public class TrendReplyDAO {

	// 오라클에 접속할때 필요한 정보들
	// DBUtil에 다 선언함.

	// 댓글 리스트를 가져오는 메서드 - 게시판 글보기 화면에서 같이 가져간다.
	public List<TrendReplyDTO> list(Integer no){
...
//			pstmt.setInt(2, pageObject.getEndRow());
			...
			while(rs.next()) {
				...
				TrendReplyDTO replyDTO = new TrendReplyDTO(rs.getInt("rno"),
	...
	// 댓글 쓰기 처리.
	public void write(TrendReplyDTO replyDTO) {
		...
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "insert into TREND_reply(rno, "
					+ " no, content, writer) "
					+ " values(TREND_reply_seq.nextval, "
					+ " ?, ?, ?) "; //변하는 데이터 대신 ? 사용
			//4. 처리문 객체
			pstmt = con.prepareStatement(sql);
	...
	// 댓글 수정 처리.
	public void update(TrendReplyDTO replyDTO) {
		...
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "update TREND_reply set "
					+ " content = ?, writer = ? "
	...
	public void delete(int rno) {
		...
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "delete from TREND_reply "
					+ " where rno = ?"; //변하는 데이터 대신 ? 사용
			//4. 처리문 객체
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rno);
