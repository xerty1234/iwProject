	...
	public List<TrendBoardDTO> list(PageObject2 pageObject){
		...
		try {
			...
			String sql = "select no, title, writer, "
					+ " writedate, hit, liked from TREND_board "
					+ search
					+ " order by no desc ";
...
//			System.out.println(idx);
	...
	public void write(TrendBoardDTO boardDTO) {
		...
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "insert into TREND_board(no,title,"
					+ " content, writer) "
	...
	public void increase(int no) {
		...
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "update TREND_board set hit = hit + 1 "
					+ " where no = ? "; //변하는 데이터 대신 ? 사용
			//4. 처리문 객체
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, no); // 첫번째 ?에 no를 int로 셋팅
	...
	public void update(TrendBoardDTO boardDTO) {
		...
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "update TREND_board set "
	...
	public void delete(int no) {
		...
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "delete from TREND_board "
					+ " where no = ?"; //변하는 데이터 대신 ? 사용
			//4. 처리문 객체
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, no);
	...
	public TrendBoardDTO getMainHighlights() {
		System.out.println("BoardDAO.view()");
		TrendBoardDTO boardDTO = null;
		// 오라클에서 데이터를 가져와서 채우는 프로그램 작성(생략)
		// 사용한 객체 선언
		...
		try {
			...
			String sql = "SELECT  no, title, content, writer, writeDate, hit, liked " + 
					" FROM TREND_board " + 
					" WHERE hit = (SELECT MAX(hit)AS hit FROM TREND_board)"; //변하는 데이터 대신 ? 사용
			//4. 처리문 객체
			pstmt = con.prepareStatement(sql);
