package com.iw.free.controller;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			case "/freeboard/list.do":
				...
				PageObject2 pageObject = new PageObject2(DBUtil.getConnection(), "FREE_BOARD", page, rowPerPage, 10,
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "/freeboard/update.do":
				...
				service = Beans.getService(command);
				// 실행해서 수정처리
				service.excute(freeDTO2);
				// 글보기로 이동시키는데 글번호와 함께 이동시킨다.
				jsp = "view.do?no="+freeDTO2.getNo()
				...
				case "/freeboard/replyWrite.do":
					...
					FReplyDTO replyDTO = new FReplyDTO(
							...
							request.getParameter("content"),
							request.getParameter("writer"));
					System.out.println("FreeController.doGet().replyDTO:"+replyDTO);
					// 처리할 서비스를 받아온다. - BoardWriteService
					service = Beans.getService(command);
					System.out.println(service);
				...
				case "/freeboard/replyUpdate.do":
					...
					System.out.println("FreeController.doGet().replyDTO:"+replyDTO2);
					// 처리할 서비스를 받아온다. - BoardWriteService
					service = Beans.getService(command);
					System.out.println(service);
