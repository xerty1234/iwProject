...
public class FReplyDAO {
	...
	public List<FReplyDTO> list(Integer no) {
		System.out.println(getClass().getName() + "FReplyDAO.list()");
		...
		try {
			...
			String sql = "select rno, no, content, writer, " + " writedate from FREPLY_BOARD " + " where no = ? "
					+ " order by rno desc ";
			...
			while (rs.next()) {
				...
				FReplyDTO replyDTO = new FReplyDTO(rs.getInt("rno"), rs.getInt("no"), rs.getString("content"),
	...
	// 댓글 쓰기 처리
	public void write(FReplyDTO replyDTO) {
		System.out.println(getClass().getName()+"FReplyDAO.write()");
		// 사용한 객체 선언
		Connection con = null; // 연결 객체
		PreparedStatement pstmt = null; // 처리문 객체
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "insert into FREPLY_BOARD(rno, "
					+ " no, content, writer) "
					+ " values(FREPLY_BOARD_seq.nextval, "
					+ " ?, ?, ?) "; //변하는 데이터 대신 ? 사용
			//4. 처리문 객체
			pstmt = con.prepareStatement(sql);
	...
	// 댓글 수정 처리.
	public void update(FReplyDTO replyDTO) {
		System.out.println(getClass().getName()+"FReplyDAO.update()");
		// 사용한 객체 선언
		Connection con = null; // 연결 객체
		PreparedStatement pstmt = null; // 처리문 객체
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "update FREPLY_BOARD set "
					+ " content = ?, writer = ? "
	...
	public void delete(int rno) {
		System.out.println(getClass().getName()+"FReplyDAO.delete()");
		// 사용한 객체 선언
		Connection con = null; // 연결 객체
		PreparedStatement pstmt = null; // 처리문 객체
		try {
			...
			con = DBUtil.getConnection();
			//3. sql문 작성
			String sql = "delete from FREPLY_BOARD "
					+ " where rno = ?"; //변하는 데이터 대신 ? 사용
			//4. 처리문 객체
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, rno);
