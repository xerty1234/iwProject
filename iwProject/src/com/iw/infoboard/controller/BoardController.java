package com.iw.infoboard.controller;
...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "/infoboard/list.do":
				...
				= new PageObject2(DBUtil.getConnection(), "INFO_board",
			...
			case "/infoboard/view.do":
				...
				break;
			// 글수정 폼 - get
			case "/infoboard/update.do":
				int no2 = Integer.parseInt(request.getParameter("no"));
				service = Beans.getService("/infoboard/view.do");//BoardViewService
				//service를 실행해서 DB에서 BoardDTO를 가져와서 request에 담는다.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "/infoboard/update.do":
				// 넘어오는 데이터를 BoardDTO에 담는다.
				infoBoardDTO boardDTO2 = new infoBoardDTO(
						Integer.parseInt(request.getParameter("no")),
						request.getParameter("title"),
						request.getParameter("content"),
						request.getParameter("writer"),null,0,0);
